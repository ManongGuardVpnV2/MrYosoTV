...
interface VideoPlayerProps {
  ...
  // optional: refresh signed/expiring streams (returns {url, expiresAt?})
    ...
    <div className="flex gap-1 mb-4">
      {'LOADING'.split('').map((letter, i) => (
        <span
          ...
          style={{ animationDelay: `${i * 0.08}s`, textShadow: '0 0 10px rgba(251,191,36,0.8)' }}
        >
          {letter}
        </span>
      ))}
...
// create a regex to match subdomains of those providers (future-proof)
const FORCE_PROXY_REGEX = new RegExp(`(^|\\.)(${FORCE_PROXY_DOMAINS.map(d => d.replace(/\./g, '\\.')).join('|')})$`, 'i');

const withBackupProxy = (url: string) => (url.startsWith(BACKUP_PROXY) ? url : `${BACKUP_PROXY}/${url}`);
const mustProxy = (url: string) => {
  try {
    const h = new URL(url).hostname;
    return FORCE_PROXY_REGEX.test(h);
  } catch { return false; }
};

const RESUME_KEY_PREFIX = 'ptv:resume:';
const RESUME_TTL = 7 * 24 * 60 * 60 * 1000;
function saveResumePosition(channelId: string, pos: number) {
  try {
    localStorage.setItem(`${RESUME_KEY_PREFIX}${channelId}`, JSON.stringify({ pos, ts: Date.now() }));
  } catch {}
}
...
/* LL-HLS probe: try to fetch master playlist and look for PART tags.
   This may fail due to CORS; we swallow errors and treat as 'no-LL' in that case. */
...
/* Identify likely stream type from URL or from a safe HEAD/GET probe (best-effort). */
async function detectStreamType(url: string): Promise<'m3u8' | 'mpd' | 'mp4' | 'youtube' | 'unknown'> {
  try {
    if (/youtube\.com|youtu\.be|youtube-nocookie/.test(url)) return 'youtube';
    if (/\.m3u8(\?|$)/i.test(url)) return 'm3u8';
    if (/\.mpd(\?|$)/i.test(url)) return 'mpd';
    if (/\.mp4(\?|$)/i.test(url)) return 'mp4';
    // best-effort fetch to detect content-type (may fail due to CORS)
    try {
      const r = await fetch(url, { method: 'GET', cache: 'no-store' });
      const ct = r.headers.get('content-type') || '';
      if (ct.includes('application/vnd.apple.mpegurl') || ct.includes('vnd.apple.mpegurl') || ct.includes('mpegurl') || ct.includes('application/x-mpegURL')) return 'm3u8';
...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ channel, onChannelChange, refreshStream }) => {
  ...
  const jwFailedRef = useRef(false);
  ...
  /* maybe refresh signed stream via provided hook */
  const maybeRefresh = useCallback(async (orig: string) => {
    try {
      if (!refreshStream) return orig;
      const r = await refreshStream(orig);
      if (r?.url) {
        if (r.expiresAt) streamExpiryRef.current = r.expiresAt;
        return r.url;
      }
    } catch (e) { console.warn('refreshStream failed', e); }
    return orig;
  }, [refreshStream]);
  ...
  const tryLoadJW = useCallback(async (streamUrl: string, ch: Channel) => {
    ...
    try { await loadJwScript(); } catch (e) { jwFailedRef.current = true; return false; }

    try {
      jwContainerRef.current.style.display = 'block';
      const final = mustProxy(streamUrl) ? withBackupProxy(streamUrl) : streamUrl;
      try {
        jwRef.current = (window as any).jwplayer(jwContainerRef.current).setup({
          file: final, autostart: true, width: '100%', height: '100%', mute: false, preload: 'auto', stretching: 'uniform'
        });
      } catch {
        try {
      ...
      jwRef.current.on?.('ready', () => {
        try {
          const resume = ch?.id ? loadResumePosition(ch.id) : null;
      ...
      jwRef.current.on?.('time', (t: any) => {
        try { if (ch?.id && t?.position != null) saveResumePosition(ch.id, Number(t.position)); } catch {}
      });
      ...
      // optional manifest poll for master -> update JW if it reveals new child master
      if (/\.m3u8/i.test(streamUrl)) {
        try {
          ...
          manifestPollRef.current = pollId as unknown as number;
        } catch {}
      }
      ...
      jwFailedRef.current = true;
      try { jwContainerRef.current && (jwContainerRef.current.style.display = 'none'); } catch {}
      return false;
    }
  }, [loadJwScript]);
  ...
  const loadMPD = useCallback(async (video: HTMLVideoElement, ch: Channel) => {
    ...
    player.configure({ streaming: { bufferingGoal: 10, rebufferingGoal: 2, bufferBehind: 30 }, abr: { enabled: true } });
    ...
    try { await player.load(ch.stream_url); } catch {
  ...
  const loadHLS = useCallback(async (video: HTMLVideoElement, ch: Channel) => {
    ...
    // prefer native if supported (Safari, iOS)
  ...
  const loadWidevine = useCallback(async (video: HTMLVideoElement, ch: Channel) => {
    ...
    const player = new shakaLib.Player(video);
    shakaRef.current = player;
    if (ch.license_url) player.configure({ drm: { servers: { 'com.widevine.alpha': ch.license_url } } });
    await player.load(ch.stream_url);
    await video.play();
  }, []);

  /* ---------------------------
     Main load logic (auto-detect + fallback cascade)
     --------------------------- */
  useEffect(() => {
    ...
    (async () => {
      ...
      // allow refresh hook for signed URLs
      try { streamUrl = await maybeRefresh(streamUrl); currentStreamRef.current = streamUrl; } catch {}

      // auto-detect type (best-effort)
      let detected: 'm3u8' | 'mpd' | 'mp4' | 'youtube' | 'unknown' = await detectStreamType(streamUrl);

      // prefer JW for many types (HLS/MP4) because jwplayer sometimes handles tricky sources better
      try {
      ...
      } catch (e) { console.warn('JW attempt failed', e); }

      try {
        if (detected === 'mpd' || channel.stream_type === 'mpd') {
          await loadMPD(video, { ...channel, stream_url: streamUrl } as Channel);
        } else if (detected === 'm3u8' || channel.stream_type === 'm3u8') {
          await loadHLS(video, { ...channel, stream_url: streamUrl } as Channel);
        } else if (channel.stream_type === 'widevine' || /widevine/i.test(String(channel.drm_type || ''))) {
          await loadWidevine(video, { ...channel, stream_url: streamUrl } as Channel);
        } else if (detected === 'youtube' || channel.stream_type === 'youtube') {
          // handled outside useEffect return -> will render iframe
          // do nothing here; parent render will return the iframe block
          setIsLoading(false);
          return;
        } else {
          // Unknown: try HLS first (many IPTV provide m3u8 even without extension),
          // then DASH, then native.
          try {
            await loadHLS(video, { ...channel, stream_url: streamUrl } as Channel);
          } catch (e1) {
            try { await loadMPD(video, { ...channel, stream_url: streamUrl } as Channel); }
            catch (e2) {
              // native fallback
              video.src = streamUrl;
              const resume = channel.id ? loadResumePosition(channel.id) : null;
              if (resume != null) try { video.currentTime = resume; } catch {}
              await video.play();
            }
          }
  ...
  }, [channel, cleanup, tryLoadJW, loadHLS, loadMPD, loadWidevine, maybeRefresh]);

  /* --------------- Signed-stream renewal (if refreshStream provided) --------------- */
  useEffect(() => {
    ...
    const schedule = () => {
      ...
      const doRenew = async () => {
        ...
        try {
          ...
          if (newData?.url) {
            ...
            // swap into active engine
            try {
              if (jwRef.current && typeof jwRef.current.load === 'function') { jwRef.current.load([{ file: newData.url }]); return; }
            } catch {}
            try {
              const HlsLib = (window as any).Hls;
              if (hlsRef.current && HlsLib && typeof hlsRef.current.loadSource === 'function') {
                try { hlsRef.current.stopLoad?.(); } catch {}
                try { hlsRef.current.loadSource(newData.url); hlsRef.current.startLoad?.(); return; } catch {}
              }
            } catch {}
            try {
      ...
      if (expiresAt && (expiresAt - Date.now()) > 1000) {
        ...
        signedRenewTimerRef.current = window.setTimeout(async () => { await doRenew(); schedule(); }, delay) as unknown as number;
      } else {
        signedRenewTimerRef.current = window.setTimeout(async () => { await doRenew(); schedule(); }, fallback) as unknown as number;
      }
    };

    schedule();

    return () => { mounted = false; try { if (signedRenewTimerRef.current) clearTimeout(signedRenewTimerRef.current); } catch {} };
  ...
  useEffect(() => {
    const v = videoRef.current;
    if (!v) return;
    const onTime = () => { setCurrentTime(v.currentTime); try { if (channel?.id) saveResumePosition(channel.id, v.currentTime); } catch {} };
  ...
  const togglePlay = useCallback(() => {
    if (jwRef.current && jwRef.current.getState) {
      try {
        const s = jwRef.current.getState();
        if (s === 'playing') jwRef.current.pause?.();
  ...
  const toggleMute = useCallback(() => {
    if (jwRef.current && typeof jwRef.current.setVolume === 'function') {
      try {
        if (isMuted) jwRef.current.setVolume(100); else jwRef.current.setVolume(0);
      } catch {}
      setIsMuted(!isMuted);
      return;
    }
  ...
  const handleVolumeChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setVolume(v);
    if (jwRef.current && typeof jwRef.current.setVolume === 'function') {
      try { jwRef.current.setVolume(Math.round(v * 100)); } catch {}
      setIsMuted(v === 0);
      return;
    }
    if (videoRef.current) {
  ...
  const handleQualityChange = (q: string) => {
    ...
    try {
      if (hlsRef.current) { if (q === 'auto') hlsRef.current.currentLevel = -1; else {
    ...
    try {
      if (shakaRef.current) {
        ...
        if (q === 'auto') shakaRef.current.configure({ abr: { enabled: true } });
          ...
          if (track) { shakaRef.current.configure({ abr: { enabled: false } }); shakaRef.current.selectVariantTrack(track, true); }
        }
      }
    } catch {}
  };

  // YouTube embed rendering (kept as earlier)
  if (channel?.stream_type === 'youtube') {
    const match = channel.stream_url.match(/(?:youtube\.com\/(?:watch\?v=|embed\/|v\/|shorts\/)|youtu\.be\/)([^"&?/ ]{11})/);
    const id = match?.[1];
    if (!id) return <div className="w-full h-full flex items-center justify-center bg-black text-red-500 font-mono">INVALID YOUTUBE URL</div>;
    ...
    return (
      <div ref={containerRef} className="relative w-full aspect-video bg-black rounded-lg overflow-hidden" style={{ boxShadow: '0 0 30px rgba(0,0,0,0.8)', border: '4px solid #5d4037' }}>
        <iframe key={id} className="w-full h-full" src={`https://www.youtube.com/embed/${id}?autoplay=1&rel=0&origin=${encodeURIComponent(origin)}`} frameBorder="0" allow="autoplay; encrypted-media" allowFullScreen />
      </div>
    );
  }

  /* ============================
     Render (UI kept identical)
     ============================ */
  return (
    <div
      ref={containerRef}
    ...
    >
      ...
      {/* native video */}
      <video ref={videoRef} className="w-full h-full object-contain" playsInline onClick={() => { if (!isLoading) togglePlay(); }} />
        ...
        <div className={`absolute inset-x-0 bottom-0 transition-all duration-300 ${showControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`} style={{ background: 'linear-gradient(to top, rgba(62,39,35,0.95), rgba(62,39,35,0.7), transparent)', borderTop: '2px solid #8d6e63' }}>
          ...
          {duration > 0 && (
            <div className="px-4 pt-2">
              <input
                ...
                style={{ background: `linear-gradient(to right,#fbbf24 0%,#fbbf24 ${(currentTime / duration) * 100}%,#5d4037 ${(currentTime / duration) * 100}%,#5d4037 100%)` }}
              />
            </div>
          )}
          ...
          <div className="flex items-center justify-between px-4 py-2">
            <div className="flex items-center gap-2 md:gap-4">
              {onChannelChange && <button onClick={() => onChannelChange('prev')} className="p-2 text-amber-200 hover:text-amber-400 transition-colors"><SkipBack className="w-5 h-5" /></button>}
              ...
              {onChannelChange && <button onClick={() => onChannelChange('next')} className="p-2 text-amber-200 hover:text-amber-400 transition-colors"><SkipForward className="w-5 h-5" /></button>}

              <div className="flex items-center gap-2">
                <button onClick={toggleMute} className="p-2 text-amber-200 hover:text-amber-400 transition-colors">{isMuted || volume === 0 ? <VolumeX className="w-5 h-5" /> : <Volume2 className="w-5 h-5" />}</button>
                <input type="range" min={0} max={1} step={0.05} value={isMuted ? 0 : volume} onChange={handleVolumeChange} className="w-16 md:w-24 h-1 appearance-none cursor-pointer rounded-full hidden sm:block" style={{ background: `linear-gradient(to right,#fbbf24 0%,#fbbf24 ${(isMuted ? 0 : volume) * 100}%,#5d4037 ${(isMuted ? 0 : volume) * 100}%,#5d4037 100%)` }} />
              </div>

              <span className="text-amber-200 text-xs md:text-sm font-mono hidden sm:block">{formatTime(currentTime)} / {formatTime(duration)}</span>
            </div>

            <div className="flex items-center gap-2">
              <div className="relative">
                <button onClick={() => setShowSettings(s => !s)} className="p-2 text-amber-200 hover:text-amber-400 transition-colors"><Settings className="w-5 h-5" /></button>
                {showSettings && (
                  <div className="absolute bottom-full right-0 mb-2 bg-amber-900/95 rounded-lg shadow-xl border border-amber-700 overflow-hidden min-w-[120px]">
                    ...
                    {availableQualities.map(q => (
                      <button key={q} onClick={() => handleQualityChange(q)} className={`w-full px-3 py-2 text-left text-sm transition-colors ${quality === q ? 'bg-amber-600 text-white' : 'text-amber-200 hover:bg-amber-800'}`}>{q}</button>
                    ))}
                  </div>
                )}
              </div>

              <button onClick={toggleFullscreen} className="p-2 text-amber-200 hover:text-amber-400 transition-colors">{isFullscreen ? <Minimize className="w-5 h-5" /> : <Maximize className="w-5 h-5" />}</button>
            </div>
          </div>
        </div>
      )}
...
/* global augmentation so TS doesn't complain */
declare global {
  interface Window { shaka: any; Hls: any; jwplayer?: any; }
}
